...
signal input_action_added(input_action: String)
signal interactable_added(input_action: String, interactable: Interactable)
...
func _physics_process(delta):
	...
	for interactable: Interactable in interactables:
		if not interactable:
			continue
		# interactable.get_input_actions()
		# Carryable
		# -> "Carry", "Drop"
		# -> "Throw": only enabled when carrying
		var input_action = interactable.required_input_action
		if _hovered_input_actions.has(input_action):
			...
			input_action_added.emit(input_action)
		interactable_added.emit(input_action, interactable)
